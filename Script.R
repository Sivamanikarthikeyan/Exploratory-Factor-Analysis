...
dataset_EFA <- read.csv("D:/SEM3 MS/R for Data Science/Feb 5 Lecture 8/New folder/dataset_EFA.csv")
View(dataset_EFA)
myData = dataset_EFA
?factanal
fit <- factanal(myData, 2)
...
fit
print(fit, digits=2, cutoff=.3, sort=TRUE)
wdbc <- read.csv("D:/SEM3 MS/R for Data Science/Feb 5 Lecture 8/New folder/wdbc.csv")
View(wdbc)
plotnScree(nS)
myData<-wdbc[,-1]
summary(myData)
...
fit <- factanal(myData, 3, lower = 0.1)
print(fit, digits=2, cutoff=.3, sort=TRUE)
fit <- factanal(myData, 6, lower = 0.1)
print(fit, digits=2, cutoff=.3, sort=TRUE)
data("bfi")
?bfi
dim(bfi)
...
print(fit, digits=2, cutoff=.3, sort=TRUE)
fit <- factanal(myData, 5)
fit <- factanal(myData, 15)
print(fit, digits=2, cutoff=.3, sort=TRUE)
fit <- factanal(myData, 5)
print(fit, digits=2, cutoff=.3, sort=TRUE)
load("D:/SEM3 MS/R for Data Science/Jan 31 Lecture 7/turkiye-student-evaluation_R_Specific.csv")
turkiye.student.evaluation_R_Specific <- read.csv("D:/SEM3 MS/R for Data Science/Jan 31 Lecture 7/turkiye-student-evaluation_R_Specific.csv")
View(turkiye.student.evaluation_R_Specific)
myData=turkiye.student.evaluation_R_Specific
dim(turkiye.student.evaluation_R_Specific)
myData<-turkiye.student.evaluation_R_Specific[,6:33]
summary(myData)
ev <- eigen(cor(myData))
ap <- parallel(subject=nrow(myData),var=ncol(myData),
rep=100,cent=.05)
...
fit <- factanal(myData, 2)
print(fit, digits=2, cutoff=.3, sort=TRUE)
fit <- factanal(myData, 21)
fit <- factanal(myData, 20)
fit <- factanal(myData, 15)
print(fit, digits=2, cutoff=.3, sort=TRUE)
fit <- factanal(myData, 10)
print(fit, digits=2, cutoff=.3, sort=TRUE)
